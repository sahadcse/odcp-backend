...
const cancelAppointment = async (req, res) => {
    try {
        ...
        const notification = new Notification({
            ...
            message: `Your appointment with ${req.doctor.full_name} on ${appointment.appointment_date.toDateString()} has been cancelled.`,
            type: 'Appointment',
            read: false,
        });
...
const getNotifications = async (req, res) => {
    ...
    try {
        const notifications = await Notification.find({ recipient: doctor_id, recipientType: 'Doctor' });
        if (!notifications.length) {
            return res.status(404).json({ message: `No notifications found- ${req.doctor.full_name}.` });
        }
...
const modifyAvailability = async (req, res) => {
    try {
        ...
        doctor.availability[availabilityIndex] = { ...doctor.availability[availabilityIndex], ...req.body };
        await doctor.save();
