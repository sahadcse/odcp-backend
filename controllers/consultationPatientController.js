...
const uploadMedicalReports = async (req, res) => {
    ...
    try {
        ...
        // Handle file uploads
        const newMedicalReports = files.medical_reports.map(file => ({ type: file.mimetype, url: file.path }));
        consultation.medical_reports = [...consultation.medical_reports, ...newMedicalReports];
...
const downloadPrescription = async (req, res) => {
    try {
        ...
        return res.status(200).json({message: 'Prescription downloaded', prescription});
    } catch (error) {
        handleError(res, error);
    }
};
...
const getNotifications = async (req, res) => {
    try {
        ...
        const notifications = await Notification.find({ recipient: patient_id, recipientType: 'Patient' });
